...
class Zre_Config {
    ...
    /**
     * Create a new configuration object, and optionally merge with an existing 
     * config.
     * 
     * @param string $mergeFilePath Optionally, merge with the config XML file at the specified file path.
     * @return Zend_Config 
     */
    public static function getDefaultConfig($mergeFilePath = '') {
	...
	$config->runmode->base_path = BASE_PATH;
	...
	$config->production->site->cryptographicSalt = uniqid(null, true);
	/**
	...
	$config->dev->merchant->paypal->api_expresscheckout_uri = 'https://www.sandbox.paypal.com/webscr';
	...
	$config->production->merchant->paypal->api_expresscheckout_uri = 'https://www.paypal.com/webscr';
	...
	$config->production->shipping->canada_post = array();
	$config->production->shipping->canada_post->origin_postal_code = 'AL 12345';

	if (!empty($mergeFilePath)) {
	    $fileExists = file_exists($mergeFilePath);

	    if ($fileExists) {
		$xml = Zre_File::read($mergeFilePath);
		$config2 = new Zend_Config_Xml($xml, null, true);
		$config2 = self::parseSettings($config, $config->runmode);
