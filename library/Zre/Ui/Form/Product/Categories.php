...
/**
 * Zre_Ui_Form_Product_Categories - Product categories form.
 *
 */
class Zre_Ui_Form_Product_Categories extends Zend_Dojo_Form
{
	public function __construct($options=null, $returnUrl='')
	{
		...
		$newButton->setAttribs(array(
			'onclick' => "JavaScript: 
				...
				frmCategories.attributes.getNamedItem('action').value = '/admin/products/category.add/';
		...
		$deleteButton->setAttribs(array(
			...
			if (dlgAllowRemove) {
				window.location = ('/admin/products/category.remove/id/' + document.getElementById('id').value);
			}" ));
