		...
		if ($is_sumbitted) {
			if ($form->isValid($this->getRequest()->getParams()) ) {
				...
				if ($is_valid == true) {
					...
					$userProfiles = $users->getProfiles(array('u.user_id' => $userAuth['user_id']));
					$userProfile = $userProfiles[0];

					$aclRole = new Zre_Dataset_Acl_Role();
					$role = $aclRole->read($userProfile['role'])->current();

					$userProfile['role'] = $role->name;

					Zre_Registry_Session::set('CURRENT_USER', $userAuth);
					Zre_Registry_Session::set('CURRENT_USER_PROFILE', $userProfile);
					Zre_Registry_Session::save();
